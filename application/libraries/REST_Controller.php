    ...
    */
    ...
    protected function _detect_api_key()
    {
        ...
        if (($key = isset($this->_args[$api_key_variable]) ? $this->_args[$api_key_variable] : $this->input->server($key_name))) {
            ...
            $this->_merchant = $this->rest->db->where('merchant_id', $key)->get('merchants')->row_array();


            if(count($this->_merchant))
            {
                define('AWS_ACCESS_KEY_ID', $this->_merchant['access_key']);
                define('AWS_SECRET_ACCESS_KEY', $this->_merchant['secret_key']);  
                define('APPLICATION_NAME', $this->_merchant['name']);
                define('APPLICATION_VERSION', '1');
                define ('MERCHANT_ID', $this->_merchant['merchant_id']);
                define ('MARKETPLACE_ID', $this->_merchant['marketplace_id']);
