...
<div class="pass-activate" id="pprotect">
    ...
    <?php $uri = $this->uri->segment(1); if(((!empty($channel_id)) && ($search_key != $channel_id) && empty($search_key)) || ($uri != 'search')) { ?>
        <a href="<?php echo base_url();?>search/<?php echo $channel_id; ?>" class="my-map pull-right" >View <small>My Map</small></a>
        <?php } else if(!empty($search_key) && ($search_key != $channel_id)) { ?>
         <a href="<?php echo base_url();?>search/<?php echo $channel_id; ?>" class="my-map pull-right" > Return to <small>My Map</small></a>
         <?php }else if($search_key == $channel_id){?>
        <a href="<?php echo site_url();?>search/<?php echo $channel_id;?>" class="my-map pull-right" > 
            My <small>Map</small>
        </a>
        <?php } else{}?>
