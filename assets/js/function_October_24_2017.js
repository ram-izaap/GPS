...
function guest_registration(type,clsname)
{
	var latlon = $("#latlang").val();
       //  alert(latlon);
        ...
        if(loctype==1){
          if(latlon=='' || latlon==0 || latlon==1 || latlon==2){
            ...
            if(add!=''){
              ...
              //latlon = $('#map_pos').val();
              latlon = (latlon!='')?latlon:$('.popover-content #guest_address').val();
        ...
        var data = {latlon:$("#latlang").val(),phone:$('#phone').val(),display:$('#display_name').val(),type:addtype};
...
function invisible_participant(user_id,grp_id,visible)
{
    ...
    $.post(site_url+'/home/invisible_participant', data, function(response){
          ...
          if(visible == 0){
            $("#invisible_icon").css("display","none");
            $("#visible_icon").css("display","block");
          }   
          else
          {
            $("#invisible_icon").css("display","block");
            $("#visible_icon").css("display","none");
           }  
